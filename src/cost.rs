...
mod tests {
    ...
    async fn test_update_rate_limits() {
        ...
        headers.insert("anthropic-ratelimit-input-tokens-limit", "400000".parse().unwrap());
        headers.insert("anthropic-ratelimit-input-tokens-remaining", "399000".parse().unwrap());
        headers.insert("anthropic-ratelimit-output-tokens-limit", "100000".parse().unwrap());
        headers.insert("anthropic-ratelimit-output-tokens-remaining", "99000".parse().unwrap());
        headers.insert("anthropic-ratelimit-tokens-reset", "2023-11-20T12:00:00Z".parse().unwrap());
        ...
        assert_eq!(status.tokens_reset, Some("2023-11-20T12:00:00Z".to_string()));
    }
}
